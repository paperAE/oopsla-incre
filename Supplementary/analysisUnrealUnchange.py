...
def collect_detect_early_data(seed_path):
        ...
        try:
            with open(file_path, 'r', newline='') as f:
                ...
                for row in reader:
                    ...
                    # Exclude 'Real' paths from the final valid set
                    if len(row) > 6 and row[6] == "Real":
    ...
    # The final set of valid paths excludes both "Real" and "Always Timed-Out" specs
...
def analyze_detect_early(file1_path):
    """
    Analyzes and prints only the "Detect Early" statistics for a group of files.
    """
    print("Processing file group...")
    valid_paths, detect_early_paths = collect_detect_early_data(file1_path)

    # The base set of paths is everything encountered (excluding Real and always-timed-out)
    ...
    print("Rule: Excludes specs that are 'Real' or time out in all runs.")
    print("-" * 50)
